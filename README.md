...
매진된 CGV 상영회차에 취소표(빈 좌석)가 생기거나, 기다리던 영화의 예매가
열리면 텔레그램/디스코드로 알림을 보낸다. GitHub Actions cron으로 무료로
돌아가며 서버가 필요 없다 (5분 주기 실행 × 실행당 1분 간격 4회 체크 ≈ 1분 감시).
...
```
/watch 용산 오디세이 0912 18:00   ← 취소표 감시 등록
/watch 용산 오디세이              ← 날짜/시간 생략 = 오픈된 전 회차
/open 아바타                      ← 예매 오픈 알림 (전국 기준)
/open 용산아이파크몰 아바타        ← 그 극장에 시간표가 뜨면 알림
/list                             ← 감시 목록
/remove 1                         ← 해제
/help                             ← 도움말
```

오픈 알림(`/open`)은 아직 예매가 안 열린 영화를 이름으로 미리 걸어두는 것으로,
한 번 울리면 자동 해제된다. 취소표 감시(`/watch`)는 예매 오픈 뒤 매진된 회차용.
